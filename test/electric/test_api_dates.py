#!/usr/bin/env python3
"""
Test whether the CPAU API respects date range parameters
"""
...
# Test different date configurations for daily mode
tests = [
    {
        'name': 'Test 1: Request 11/21-11/23 (3 days)',
        'payload': {
            'UsageOrGeneration': '1',
            'Type': 'K',
            'Mode': 'D',
    ...
    {
        'name': 'Test 2: Request single day 11/22/25',
        'payload': {
            'UsageOrGeneration': '1',
            'Type': 'K',
            'Mode': 'D',
            ...
            'usageyear': '',
            'MeterNumber': meter_number,
            'DateFromDaily': '11/22/25',
            'DateToDaily': '11/22/25',
            'IsTier': True,
            'IsTou': False
        }
    },
    {
        'name': 'Test 3: Empty date fields',
        'payload': {
            'UsageOrGeneration': '1',
            'Type': 'K',
            'Mode': 'D',
...
for test in tests:
    print("="*80)
    print(f"{test['name']}")
    print(f"Payload dates: From={test['payload']['DateFromDaily']}, To={test['payload']['DateToDaily']}")
    ...
    if response.status_code == 200:
        ...
        print(f"Date range in response: {min(usage_dates) if usage_dates else 'N/A'} to {max(usage_dates) if usage_dates else 'N/A'}")
