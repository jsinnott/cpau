...
from watersmart_session import WatersmartSessionManager

logger = logging.getLogger(__name__)


# Temporary UsageRecord for development - will use the one from cpau.meter
from dataclasses import dataclass

@dataclass
class UsageRecord:
    """
    Represents a single water usage data point.

    Note: Reusing electric meter field names for consistency, but values represent:
    - import_kwh: Water consumption in gallons (not kWh!)
    - export_kwh: Always 0 (water doesn't export)
    - net_kwh: Same as import_kwh (total consumption)
    """
    date: datetime
    import_kwh: float  # Actually gallons for water meter
    export_kwh: float  # Always 0 for water
    net_kwh: float     # Same as import_kwh
...
if __name__ == '__main__':
    ...
    # Test hourly data
    print("\n" + "=" * 70)
    print("Testing Hourly Usage")
    print("=" * 70)

    test_start = date(2024, 12, 1)
    test_end = date(2024, 12, 3)

    hourly = meter.get_hourly_usage(test_start, test_end)
    print(f"\nRetrieved {len(hourly)} hourly records from {test_start} to {test_end}")

    if hourly:
        print(f"\nFirst record: {hourly[0]}")
        print(f"Last record: {hourly[-1]}")

        total_gallons = sum(r.import_kwh for r in hourly)
        print(f"\nTotal consumption: {total_gallons:.2f} gallons")

    # Test daily data
    print("\n" + "=" * 70)
    print("Testing Daily Usage")
    print("=" * 70)

    daily_start = date(2024, 11, 1)
    daily_end = date(2024, 11, 30)

    daily = meter.get_daily_usage(daily_start, daily_end)
    print(f"\nRetrieved {len(daily)} daily records from {daily_start} to {daily_end}")

    if daily:
        print(f"\nFirst record: {daily[0]}")
        print(f"Last record: {daily[-1]}")

        total_gallons = sum(r.import_kwh for r in daily)
        print(f"\nTotal consumption: {total_gallons:.2f} gallons")
        print(f"Average daily: {total_gallons/len(daily):.2f} gallons/day")

    # Test billing data
    print("\n" + "=" * 70)
    print("Testing Billing Usage")
    print("=" * 70)

    billing_start = date(2024, 1, 1)
    billing_end = date(2024, 12, 31)

    billing = meter.get_billing_usage(billing_start, billing_end)
    print(f"\nRetrieved {len(billing)} billing periods from {billing_start} to {billing_end}")

    if billing:
        print(f"\nFirst period: {billing[0]}")
        print(f"Last period: {billing[-1]}")

        total_gallons = sum(r.import_kwh for r in billing)
        print(f"\nTotal consumption (2024): {total_gallons:.2f} gallons")
    ...
    monthly_start = date(2024, 9, 1)
    monthly_end = date(2024, 11, 30)

    monthly = meter.get_monthly_usage(monthly_start, monthly_end)
    print(f"\nRetrieved {len(monthly)} monthly records from {monthly_start} to {monthly_end}")

    if monthly:
        print(f"\nFirst month: {monthly[0]}")
        print(f"Last month: {monthly[-1]}")

        total_gallons = sum(r.import_kwh for r in monthly)
        print(f"\nTotal consumption (Sep-Nov 2024): {total_gallons:.2f} gallons")
        print(f"Average monthly: {total_gallons/len(monthly):.2f} gallons/month")

        # Show all months
        print("\nMonthly breakdown:")
        for record in monthly:
            month_name = record.date.strftime('%B %Y')
            print(f"  {month_name}: {record.import_kwh:.2f} gallons")

    # Test availability window
    print("\n" + "=" * 70)
    print("Testing Data Availability Windows")
    print("=" * 70)

    for interval in ['hourly', 'daily', 'billing', 'monthly']:
        earliest, latest = meter.get_availability_window(interval)
        if earliest and latest:
            print(f"\n{interval.capitalize():10s}: {earliest} to {latest}")
            days = (latest - earliest).days + 1
            print(f"{' ' * 10}  ({days} days of data)")
        else:
            print(f"\n{interval.capitalize():10s}: No data available")
